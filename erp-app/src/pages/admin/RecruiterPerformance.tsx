...
const RecruiterPerformance: React.FC = () => {
  ...
  const getPerformanceColor = (value: number, type: 'db' | 'meeting' | 'contract') => {
    if (viewMode === 'average') {
      // 평균 기준
  ...
  const totalStats = performanceData.reduce(
    ...
    { total_db: 0, meeting_completed: 0, contract_completed: 0 }
  );
  ...
  return (
    <div className="p-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800">섭외자 개인별 실적</h1>
        <p className="text-gray-600 mt-2">섭외자들의 실적을 한눈에 확인하세요</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <p className="text-xs opacity-80 mt-1">
            {viewMode === 'average' && performanceData.length > 0 
              ? `월평균 ${formatNumber(totalStats.total_db / performanceData.length)}` 
              : ''}
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <p className="text-xs opacity-80 mt-1">
            {viewMode === 'average' && performanceData.length > 0 
              ? `월평균 ${formatNumber(totalStats.meeting_completed / performanceData.length)}` 
              : ''}
          </p>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <p className="text-xs opacity-80 mt-1">
            {viewMode === 'average' && performanceData.length > 0 
              ? `월평균 ${formatNumber(totalStats.contract_completed / performanceData.length)}` 
              : ''}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-xl font-semibold text-gray-800">섭외자별 상세 실적</h2>
